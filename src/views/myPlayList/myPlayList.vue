  ...
  <main class="rank" v-if="myCreadPlayList">
    <div class="r-wrap">
      <div class="m-rank">
        <div class="l-r-wrap">
          <h2>
            创建的歌单<span>({{ myCreadPlayList.length }})</span>
          </h2>
          <ul>
            <li
              v-for="(item, index) in myCreadPlayList"
              :key="item.id"
              :class="
          ...
          <h2 class="h-height">
            收藏的歌单<span>({{ orderPlayList.length }})</span>
          </h2>
          <ul>
            <li
              class="clearFix"
              v-for="(item2, index) in orderPlayList"
              :key="item2.id"
              @click="changePlayList(item2.id)"
              :class="
                item2.id == songDetail.id ? 'l-r-active clearFix' : 'clearFix'
              "
            >
              <div class="l-r-spelist">
                <a href="javascript:;" class="l-r-left"
                  ><img :src="item2.coverImgUrl + '?param=40y40'"
                /></a>
                <div class="l-r-right">
                  <p>
                    <a>{{ item2.name }}</a>
                  ...
                  <span
                    >{{ item2.trackCount }}首 by
                    <span>{{ item2.nickname }}</span></span
                  >
                </div>
              </div>
            </li>
      ...
      <div class="r-rank clearFix">
        <div class="r-t-wrap">
          ...
          <div class="r-icloud">
            ...
            <p>
              <img :src="songDetail.avatarUrl" width="36" alt="" /><span
                style="color: #0c73c2"
                >{{ songDetail.nickname }}</span
              ><span class="i-color"
                >{{
                  $moment(songDetail.updateTime).format("YYYY-MM-DD")
                }}
                创建</span
              >
            </p>
          ...
          <div class="icloud-play clearFix">
            <a class="i-play" @click="playSongList()">
              <span><i></i>播放</span>
            </a>
            <a class="i-add"></a>
            <a class="i-add2">
        ...
        <section class="song-list">
          ...
          <table>
            ...
            <tr v-for="(item3, index3) in songTable" :key="item3.id">
              <td>
              ...
              <td>
                ...
                <nav class="play-nav">
                  <a class="m-add"></a>
                  <a class="m-collection"></a>
                  <a class="m-share"></a>
                  <a class="m-download"></a>
                </nav>
              </td>
              <td>
                <a class="music-singer">{{ item3.al }}</a>
              </td>
            </tr>
          </table>
        </section>
          ...
          <div class="comarea clearFix">
            ...
            <div class="commentarea">
              ...
              <div class="comicon">
                <i class="smile"></i>
                <i class="art"></i>
                <a class="comtxt" @click="sendCommentByid">评论</a>
                <span>140</span>
              </div>
            </div>
          </div>
              ...
              >
                ...
                <div class="com-content">
                  ...
                  <p class="com-bottom">
                    ...
                    <a class="finger"><i></i>({{ item5.likedCount }})</a>
                    <span>|</span>
                    <a>回复</a>
                  </p>
                </div>
...
export default {
  ...
  mounted() {
    ...
    this.user = user;
    this.userPlayListInfo.uid = user.id;
    this.userPlayListInfo.cookie = user.cookie;
    //获取用户的歌单
    this.$store.dispatch("getUserPlayList", this.userPlayListInfo);

    setTimeout(() => {
      //获取歌单详情
      this.playDetailInfo.cookie = this.user.cookie;
      this.playDetailInfo.id = this.myCreadPlayList[0].id;
      this.$store.dispatch("PlayUserList", this.playDetailInfo);
  ...
  computed: {
    ...mapState({
      userPlayList: (state) => state.userPlayList.playlist,
      playList: (state) => state.playList.playlist,
      comments: (state) => state.comments.data,
    }),
    //我创建的歌单
    myCreadPlayList: function () {
      let arr;
      if (typeof this.userPlayList != "undefined") {
        arr = this.userPlayList.map((item, index) => {
  ...
  watch: {
    playList(newVal, oldVal) {
      ...
      this.songDetail.nickname = newVal.creator.nickname;
      this.songDetail.avatarUrl = newVal.creator.avatarUrl;
